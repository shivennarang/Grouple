...
import { onAddCustomDomain, onDeleteGroupGallery, onGetAllGroupMembers, onGetAllUserMessages, onGetDomainConfig, onGetExploreGroup, onGetGroupInfo, onSearchGroups, onSendMessage, onUpdateGroupGallery, onUpDateGroupSettings } from "@/actions/groups"
import { AddCustomDomainSchema } from "@/components/forms/domain/schema"
import { GroupSettingsSchema } from "@/components/forms/group-settings/schems"
...
import { onChat } from "@/redux/slices/chats-slice"
import { onClearList, onInfiniteScroll } from "@/redux/slices/infinite-scroll"
...
export const useGroupAbout = (
  ...
  const deleteMediaItem = async (mediaId: string) => {
    try {
      const mediaType = validateURLString(mediaId).type;
      console.log("mediadid=",mediaId);
      if (mediaType === "IMAGE" || mediaType === "YOUTUBE" || mediaType === "LOOM") {
        const deleteResult = await onDeleteGroupGallery(groupid, mediaId);
        ...
        if (deleteResult?.status === 200) {
          ...
          toast("Error", {
            description: deleteResult?.message || "Failed to delete media item",
          });
        }
      } else {
        toast("Error", {
