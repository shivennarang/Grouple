...
export const onDeleteGroupGallery = async (groupId: any, mediaItem: any) => {
  try {
    console.log("mediaid",mediaItem);
    // Get the current gallery for the group
    const group = await client.group.findUnique({
      where: { id: groupId },
      select: { gallery: true },
    });

    if (!group || !group.gallery) {
      return { status: 404, message: "Group or gallery not found" };
    }

    // Validate media type to ensure it's a supported type (IMAGE, YOUTUBE, LOOM)
    const mediaType = validateURLString(mediaItem).type;
    if (mediaType === "IMAGE" || mediaType === "YOUTUBE" || mediaType === "LOOM") {
      
      // Filter out the media item to be deleted
      const updatedGallery = group.gallery.filter(item => item !== mediaItem);
      ...
      return { status: 200, message: "Media item deleted successfully" };
    } else {
      return { status: 400, message: "Unsupported media type" };
    }
  } catch (error) {
    console.error("Error deleting media item from gallery:", error);
    return { status: 400, message: "Failed to delete media item" };
  }
};
